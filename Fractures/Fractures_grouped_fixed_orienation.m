function f = Fractures_grouped_fixed_orienation(Dom, NumGroups, NUM_fracs, len, orientation)
    Frac(1) = struct('tag', 0, ...
    ...
    for s = 1:1:NumGroups
        ...
        for j = 1:NUM_fracs(s)
            ...
            normal_x = orientation(s, 1); % random value within [-1, 1]
            normal_y = orientation(s, 2);
            ...
            Frac(i).orientation_normal = Frac(i).orientation_normal ./ norm(Frac(i).orientation_normal);
            ...
            Frac(i).length = len(s, 1) + (len(s, 2) - len(s, 1)) * rand;
