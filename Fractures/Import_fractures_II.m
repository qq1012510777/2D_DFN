function f = Import_fractures_II(S)
    Frac(1) = struct('tag', 0, ...
    ...
    Num_Fracs = size(S, 1) / 4;

    for i = 1:1:Num_Fracs
        % the tag
        Frac(i).tag = i;

        eval(['Frac(i).ends_x(1, 1) = S(', num2str((i - 1) * 4 + 1), ', 1);']);
        eval(['Frac(i).ends_y(1, 1) = S(', num2str((i - 1) * 4 + 1), ', 3);']);
        eval(['Frac(i).ends_x(1, 2) = S(', num2str((i - 1) * 4 + 4), ', 1);']);
        eval(['Frac(i).ends_y(1, 2) = S(', num2str((i - 1) * 4 + 4), ', 3);']);
